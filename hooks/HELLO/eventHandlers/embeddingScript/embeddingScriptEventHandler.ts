...
const useHandleHelloEmbeddingScriptEvents = (eventHandler: EmbeddingScriptEventRegistryInstance) => {
    ...
    const handleHelloData = (event: MessageEvent) => {
        ...
        const finalClientInfo = { mail, number, name, ...(cleanObject(existingClientInfo || {})), ...(restProps || {}) }

        setLocalStorage('clientData', JSON.stringify(finalClientInfo));
        setLocalStorage('userData', JSON.stringify({ unique_id, name, mail: finalClientInfo?.mail, number: finalClientInfo?.number, user_jwt_token: hasUserIdentity ? user_jwt_token : undefined }));
