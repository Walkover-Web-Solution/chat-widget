...
export const chatReducerV2 = {
    ...
    removeMessages: (state, action: PayloadAction<{ numberOfMessages: number }>) => {
        const { numberOfMessages } = action.payload;
        const subThreadId = state.subThreadId;

        if (subThreadId && state.messageIds[subThreadId]) {
            ...
            messageIdsToRemove.forEach(msgId => {
                if (state.msgIdAndDataMap[subThreadId]) {
                    delete state.msgIdAndDataMap[subThreadId][msgId];
                }
            });

            // Update message IDs array
            state.messageIds[subThreadId] = state.messageIds[subThreadId].slice(0, -numberOfMessages);
        }
    },
