    ...
    useEffect(() => {
        const handleNotificationMessage = (event: MessageEvent) => {
            if (event?.data?.type === 'OPEN_WITH_NOTIFICATION') {
                ...
                // Also update the active subThreadId used for view routing (state.Chat.subThreadId),
                // otherwise the chat view keeps showing whatever thread/team was last active.
                dispatch(setSubThreadId(newSubThreadId));
                // Close the side drawer so the newly opened chat is visible
                dispatch(setToggleDrawer(false));
                // The notification has been opened by the user, so mark it read (remove it)
                if (notificationId) {
